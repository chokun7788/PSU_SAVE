# Ad Hoc Pipeline Test Log

- Generated: 2026-07-04T23:58:54
- JSONL: `C:\Users\Chokhun\Downloads\Learn-LLM\18_PSU_Esports_Update_Route_Data\reports\ad_hoc_pipeline_results_equipment_game_catalog_fix5_source_20260704.jsonl`
- Total questions: 21

## Summary

- competition_rules/competition_rules_lookup: 2
- equipment/equipment_game_catalog: 10
- equipment/zone_equipment_lookup: 2
- games/competition_game_list: 1
- games/game_availability_lookup: 5
- games/game_detail_lookup: 1

## Results

### 1. อุปกรณ์เล่นเกมอะไรได้บ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0054` sec

คำตอบ:

สรุปเกมที่เล่นได้ตามอุปกรณ์/โซนของ PSU Esports Studio - Phuket:
- PC Zone: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
- PlayStation 5 Zone: Marvel's Spider-Man 2, TEKKEN 8, Fortnite, God of War Ragnarok และเกม PlayStation 5 อื่น ๆ ในรายการ
- Nintendo Switch Zone: Mario Kart 8 Deluxe, Overcooked 2, Super Smash Bros Ultimate, Nintendo Switch Sports และเกม Nintendo Switch อื่น ๆ ในรายการ
- Cockpit Zone: Gran Turismo 7 (Single Player)
- VR Zone: Beat Saber และ Horizon Call of the Mountain
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 2. อุปกรณ์มีเกมอะไรบ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0036` sec

คำตอบ:

สรุปเกมที่เล่นได้ตามอุปกรณ์/โซนของ PSU Esports Studio - Phuket:
- PC Zone: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
- PlayStation 5 Zone: Marvel's Spider-Man 2, TEKKEN 8, Fortnite, God of War Ragnarok และเกม PlayStation 5 อื่น ๆ ในรายการ
- Nintendo Switch Zone: Mario Kart 8 Deluxe, Overcooked 2, Super Smash Bros Ultimate, Nintendo Switch Sports และเกม Nintendo Switch อื่น ๆ ในรายการ
- Cockpit Zone: Gran Turismo 7 (Single Player)
- VR Zone: Beat Saber และ Horizon Call of the Mountain
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 3. เครื่องเล่นอะไรได้บ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0042` sec

คำตอบ:

สรุปเกมที่เล่นได้ตามอุปกรณ์/โซนของ PSU Esports Studio - Phuket:
- PC Zone: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
- PlayStation 5 Zone: Marvel's Spider-Man 2, TEKKEN 8, Fortnite, God of War Ragnarok และเกม PlayStation 5 อื่น ๆ ในรายการ
- Nintendo Switch Zone: Mario Kart 8 Deluxe, Overcooked 2, Super Smash Bros Ultimate, Nintendo Switch Sports และเกม Nintendo Switch อื่น ๆ ในรายการ
- Cockpit Zone: Gran Turismo 7 (Single Player)
- VR Zone: Beat Saber และ Horizon Call of the Mountain
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 4. เครื่องเล่นเกมอะไรได้บ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0041` sec

คำตอบ:

สรุปเกมที่เล่นได้ตามอุปกรณ์/โซนของ PSU Esports Studio - Phuket:
- PC Zone: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
- PlayStation 5 Zone: Marvel's Spider-Man 2, TEKKEN 8, Fortnite, God of War Ragnarok และเกม PlayStation 5 อื่น ๆ ในรายการ
- Nintendo Switch Zone: Mario Kart 8 Deluxe, Overcooked 2, Super Smash Bros Ultimate, Nintendo Switch Sports และเกม Nintendo Switch อื่น ๆ ในรายการ
- Cockpit Zone: Gran Turismo 7 (Single Player)
- VR Zone: Beat Saber และ Horizon Call of the Mountain
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 5. อุปกรณ์นี้เล่นไรได้บ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0035` sec

คำตอบ:

สรุปเกมที่เล่นได้ตามอุปกรณ์/โซนของ PSU Esports Studio - Phuket:
- PC Zone: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
- PlayStation 5 Zone: Marvel's Spider-Man 2, TEKKEN 8, Fortnite, God of War Ragnarok และเกม PlayStation 5 อื่น ๆ ในรายการ
- Nintendo Switch Zone: Mario Kart 8 Deluxe, Overcooked 2, Super Smash Bros Ultimate, Nintendo Switch Sports และเกม Nintendo Switch อื่น ๆ ในรายการ
- Cockpit Zone: Gran Turismo 7 (Single Player)
- VR Zone: Beat Saber และ Horizon Call of the Mountain
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 6. PC Zone เล่นเกมอะไรได้บ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0064` sec

คำตอบ:

สรุปเกมที่เล่นได้ในโซนที่ถาม:
- PC Zone: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
  อุปกรณ์หลัก: Gaming PC รุ่น MSI MAG Infinite S3 14th 10 เครื่อง (10 Units), Gaming Monitor 10 เครื่อง (10 Units), Gaming Chair 10 ตัว (10 Units), Gaming Keyboard, Gaming Mouse และ Gaming Headset
  วิธีใช้งานโดยสรุป: จอง PC Zone ตามรอบบริการ เข้าใช้งานเครื่องที่ศูนย์จัดไว้ แล้วเปิดเกมหรือโปรแกรมที่ต้องการตามรายการที่มี
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 7. PC Zone มีอุปกรณ์อะไรบ้าง

- mode: `pipeline:equipment_zone_fast_path`
- route: `equipment/zone_equipment_lookup`
- confidence: `0.95`
- elapsed: `0.0075` sec

คำตอบ:

PC Zone คือโซนคอมพิวเตอร์เกมมิ่งสำหรับเล่นเกมบน PC และใช้ฝึกซ้อม/เรียนรู้ด้านอีสปอร์ต
วิธีใช้งานโดยสรุป: จอง PC Zone ตามรอบบริการ เข้าใช้งานเครื่องที่ศูนย์จัดไว้ แล้วเปิดเกมหรือโปรแกรมที่ต้องการตามรายการที่มี
อุปกรณ์หลัก: Gaming PC รุ่น MSI MAG Infinite S3 14th 10 เครื่อง (10 Units), Gaming Monitor 10 เครื่อง (10 Units), Gaming Chair 10 ตัว (10 Units), Gaming Keyboard, Gaming Mouse และ Gaming Headset
เกม/การใช้งานที่ยืนยันได้: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/home

แหล่งข้อมูล:
- home | https://esports.phuket.psu.ac.th/home

### 8. Cockpit มีเกมอะไรบ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0055` sec

คำตอบ:

สรุปเกมที่เล่นได้ในโซนที่ถาม:
- Cockpit Zone: Gran Turismo 7 (Single Player)
  อุปกรณ์หลัก: TV 65 นิ้ว 2 เครื่อง (2 Units), Logitech G923 TRUEFORCE Racing wheel + Driving Force Shifter 2 ชุด (2 Units), Racezone Full Cockpit V3 2 ชุด (2 Units) และ Pulse Elite Wireless Headset 2 ชุด (2 Units)
  วิธีใช้งานโดยสรุป: จอง Cockpit Zone แล้วนั่งในชุด Cockpit ใช้พวงมาลัย Logitech G923 พร้อมคันเร่ง/เบรก/คันเกียร์เพื่อควบคุมรถในเกม
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 9. VR มีเกมอะไรบ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0053` sec

คำตอบ:

สรุปเกมที่เล่นได้ในโซนที่ถาม:
- VR Zone: Beat Saber และ Horizon Call of the Mountain
  อุปกรณ์หลัก: PlayStation 5 Slim With Ultra HD Blu-Ray Disc Drive 1 เครื่อง (1 Unit) และ Sony PlayStation VR2 1 ชุด (1 Unit)
  วิธีใช้งานโดยสรุป: จอง VR Zone แล้วสวม PlayStation VR2 ใช้คอนโทรลเลอร์ VR ตามคำแนะนำของเกมหรือเจ้าหน้าที่ และควรเล่นในพื้นที่ที่จัดไว้เท่านั้น
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 10. PS5 มีเกมอะไรบ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.0044` sec

คำตอบ:

สรุปเกมที่เล่นได้ในโซนที่ถาม:
- PlayStation 5 Zone: Marvel's Spider-Man 2, TEKKEN 8, Fortnite, God of War Ragnarok และเกม PlayStation 5 อื่น ๆ ในรายการ
  อุปกรณ์หลัก: PlayStation 5 Slim With Ultra HD Blu-Ray Disc Drive 2 เครื่อง (2 Units)
  วิธีใช้งานโดยสรุป: จอง PlayStation 5 Zone แล้วเล่นเกมผ่านเครื่อง PS5 และจอ/อุปกรณ์ที่ศูนย์จัดไว้ เลือกเกมตามรายการที่มีให้บริการ
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 11. Nintendo Switch มีเกมอะไรบ้าง

- mode: `pipeline:equipment_game_catalog_fast_path`
- route: `equipment/equipment_game_catalog`
- confidence: `0.96`
- elapsed: `0.006` sec

คำตอบ:

สรุปเกมที่เล่นได้ในโซนที่ถาม:
- Nintendo Switch Zone: Mario Kart 8 Deluxe, Overcooked 2, Super Smash Bros Ultimate, Nintendo Switch Sports และเกม Nintendo Switch อื่น ๆ ในรายการ
  อุปกรณ์หลัก: TV 86 นิ้ว 1 เครื่อง (1 Unit), Sofa 2 seats 2 ชุด (2 Units) และ Nintendo Switch OLED 1 เครื่อง (1 Unit)
  วิธีใช้งานโดยสรุป: จอง Nintendo Switch Zone แล้วเล่นผ่าน Nintendo Switch OLED กับ TV 86 นิ้ว ใช้จอย Joy-Con/Controller ตามเกมที่เลือก
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 12. คอมมีวาโลไหม

- mode: `pipeline:games_availability_fast_path`
- route: `games/game_availability_lookup`
- confidence: `0.95`
- elapsed: `0.0038` sec

คำตอบ:

มีครับ VALORANT อยู่ในรายการเกมที่ยืนยันได้ของ PC Zone
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 13. เพลย์ห้ามี tekken 8 หรือเปล่า

- mode: `pipeline:games_availability_fast_path`
- route: `games/game_availability_lookup`
- confidence: `0.95`
- elapsed: `0.0107` sec

คำตอบ:

มีครับ TEKKEN 8 อยู่ในรายการเกมที่ยืนยันได้ของ PC Zone และ PlayStation 5 Zone
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 14. Warzone อยู่เครื่อง PC ไหน

- mode: `pipeline:games_availability_fast_path`
- route: `games/game_availability_lookup`
- confidence: `0.95`
- elapsed: `0.011` sec

คำตอบ:

มีครับ Call of Duty: Warzone อยู่ในรายการเกมที่ยืนยันได้ของ PC Zone
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 15. Tekken 8 เกมนึงมี 3 rounds ใช่ไหม

- mode: `pipeline:competition_fact_card`
- route: `competition_rules/competition_rules_lookup`
- confidence: `0.96`
- elapsed: `0.034` sec

คำตอบ:

คำตอบ: Tekken 8 แต่ละเกมตั้งค่าเป็น 3 รอบ (R3)

หลักฐานจากกติกา:
- เอกสารการตั้งค่าเกมระบุ Round 3 / R3

อ้างอิงจากกติกา: Tekken 8 / PSU Esports ปะทะมันส์ สนั่นจอ
แหล่งข้อมูล: local://competition_rules/competition_rules_tekken8_psu_esports

แหล่งข้อมูล:
- tekken8_rounds3_challenger | local://competition_rules/competition_rules_tekken8_psu_esports

### 16. tekken 1 ต่อ 1 ในเกมรวมต้องมี decider หรือเปล่า

- mode: `pipeline:competition_fact_card`
- route: `competition_rules/competition_rules_lookup`
- confidence: `0.96`
- elapsed: `0.0266` sec

คำตอบ:

คำตอบ: Tekken 8 หากเสมอกัน 1-1 ต้องเล่นเกมตัดสินเพื่อหาผู้ชนะ

หลักฐานจากกติกา:
- รูปแบบ FT2 ทำให้เมื่อเสมอ 1-1 ต้องมีเกมตัดสินเพื่อให้มีผู้ชนะครบ 2 เกม

อ้างอิงจากกติกา: Tekken 8 / PSU Esports ปะทะมันส์ สนั่นจอ
แหล่งข้อมูล: local://competition_rules/competition_rules_tekken8_psu_esports

แหล่งข้อมูล:
- tekken8_decider_challenger | local://competition_rules/competition_rules_tekken8_psu_esports

### 17. เล่น Minecraft ได้ไหม

- mode: `pipeline:games_unknown_fast_path`
- route: `games/game_availability_lookup`
- confidence: `0.94`
- elapsed: `0.0074` sec

คำตอบ:

ยังไม่พบ Minecraft ในรายการเกมที่ยืนยันได้ของ PSU Esports Studio - Phuket ครับ
ถ้าต้องการเล่นเกมนอกเหนือจากรายการนี้ ควรสอบถามเจ้าหน้าที่ก่อนจองหรือก่อนเข้าใช้บริการ

เกมที่มีข้อมูลยืนยันตอนนี้:
- PC Zone: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
- PlayStation 5 Zone: Marvel's Spider-Man 2, TEKKEN 8, Fortnite, God of War Ragnarok และเกม PS5 อื่น ๆ ในรายการ
- Nintendo Switch Zone: Mario Kart 8 Deluxe, Overcooked 2, Super Smash Bros Ultimate, Nintendo Switch Sports และเกม Switch อื่น ๆ ในรายการ
- Cockpit Zone: Gran Turismo 7
- VR Zone: Beat Saber และ Horizon Call of the Mountain
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 18. Roblox เล่นได้ไหม

- mode: `pipeline:games_unknown_fast_path`
- route: `games/game_availability_lookup`
- confidence: `0.94`
- elapsed: `0.0051` sec

คำตอบ:

ยังไม่พบ Roblox ในรายการเกมที่ยืนยันได้ของ PSU Esports Studio - Phuket ครับ
ถ้าต้องการเล่นเกมนอกเหนือจากรายการนี้ ควรสอบถามเจ้าหน้าที่ก่อนจองหรือก่อนเข้าใช้บริการ

เกมที่มีข้อมูลยืนยันตอนนี้:
- PC Zone: VALORANT, Counter-Strike 2, PUBG: BATTLEGROUNDS, Call of Duty: Warzone, TEKKEN 8 และ League of Legends
- PlayStation 5 Zone: Marvel's Spider-Man 2, TEKKEN 8, Fortnite, God of War Ragnarok และเกม PS5 อื่น ๆ ในรายการ
- Nintendo Switch Zone: Mario Kart 8 Deluxe, Overcooked 2, Super Smash Bros Ultimate, Nintendo Switch Sports และเกม Switch อื่น ๆ ในรายการ
- Cockpit Zone: Gran Turismo 7
- VR Zone: Beat Saber และ Horizon Call of the Mountain
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 19. Beat Saber เล่นยังไง

- mode: `pipeline:game_detail_fast_path`
- route: `games/game_detail_lookup`
- confidence: `0.96`
- elapsed: `0.0064` sec

คำตอบ:

Beat Saber: Beat Saber คือเกม VR จังหวะดนตรีที่ผู้เล่นใช้ดาบแสงฟันบล็อกตามจังหวะเพลง
แนวเกม: เกม VR Rhythm
วิธีเล่นโดยสรุป: สวมแว่น VR ถือคอนโทรลเลอร์ แล้วฟันบล็อกตามทิศทาง หลบสิ่งกีดขวาง และพยายามทำคะแนนตามจังหวะเพลง
เล่นได้ที่: VR Zone
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/Services/our-games

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation

### 20. อะไรคือ Cockpit

- mode: `pipeline:equipment_zone_fast_path`
- route: `equipment/zone_equipment_lookup`
- confidence: `0.95`
- elapsed: `0.004` sec

คำตอบ:

Cockpit Zone คือโซนจำลองการขับรถ/เกมแข่งรถ ใช้เล่น Gran Turismo 7 ด้วยชุดพวงมาลัยและเบาะขับ
วิธีใช้งานโดยสรุป: จอง Cockpit Zone แล้วนั่งในชุด Cockpit ใช้พวงมาลัย Logitech G923 พร้อมคันเร่ง/เบรก/คันเกียร์เพื่อควบคุมรถในเกม
อุปกรณ์หลัก: TV 65 นิ้ว 2 เครื่อง (2 Units), Logitech G923 TRUEFORCE Racing wheel + Driving Force Shifter 2 ชุด (2 Units), Racezone Full Cockpit V3 2 ชุด (2 Units) และ Pulse Elite Wireless Headset 2 ชุด (2 Units)
เกม/การใช้งานที่ยืนยันได้: Gran Turismo 7 (Single Player)
แหล่งข้อมูล: https://esports.phuket.psu.ac.th/home

แหล่งข้อมูล:
- home | https://esports.phuket.psu.ac.th/home

### 21. ตอนนี้มีเกมแข่งอะไรบ้าง

- mode: `pipeline:competition_game_list_fast_path`
- route: `games/competition_game_list`
- confidence: `0.95`
- elapsed: `0.0044` sec

คำตอบ:

เกม/รายการแข่งขันที่มีข้อมูลกติกาในฐานข้อมูลตอนนี้:
- Counter-Strike 2: PSU Phuket CS2 2026 Tournament
- VALORANT: PSU Phuket VALORANT 2026 Tournament
- Arena of Valor (RoV): Blueket Games 2025 ประเภททีมชาย
- TEKKEN 8: PSU Esports / Tekken 8 Tournament
หมายเหตุ: นี่คือรายการที่มีเอกสารกติกาหรือข้อมูลการแข่งขันในฐานข้อมูล ไม่ได้ยืนยันว่าเปิดรับสมัครอยู่ตอนนี้
แหล่งข้อมูล: data/competition_rules

แหล่งข้อมูล:
- our_games | https://esports.phuket.psu.ac.th/Services/our-games
- Reservation | https://esports.computing.psu.ac.th/reservation
